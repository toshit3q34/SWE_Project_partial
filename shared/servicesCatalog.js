/**
 * HMIS G3 service catalog — aligned with *HMIS G3 Services Technical Documentation* (March 31, 2026).
 * Client source of truth. Server mirrors this in `server/src/data/hmisServicesCatalog.js` — keep both in sync.
 */
...
/** @type {{ id: string, name: string, services: { code: string, name: string, description: string, path: string }[] }[] }} */
...
export function hmisServiceCodes() {
  return HMIS_SERVICE_CATEGORIES.flatMap((c) => c.services.map((s) => s.code));
}
